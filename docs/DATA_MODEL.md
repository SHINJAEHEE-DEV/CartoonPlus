...
CSV 입력은 Caspio 형식을 사용한다. 파서가 제목·권수·서가를 분리하고, `upsert_inventory`가 제목+작가의 도서 마스터와 지점별 재고를 갱신한다. 같은 지점·같은 도서는 중복 생성하지 않으며, CSV에 없는 기존 재고를 자동 삭제하지 않는다. 보관은 `archived_at`으로 처리한다.
