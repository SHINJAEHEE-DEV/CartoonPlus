...
서울대입구역점 CSV는 Caspio 형식(`title`, `number`, `genre`, `author`)을 사용한다. 잠실점 CSV는 서가 형식(`a_`, `a___`)으로, 한 `a___` 값의 `//` 묶음을 개별 제목으로 나눈다. 두 형식 모두 끝의 공백+숫자만 권수로 분리해 `1~N권`으로 저장하고 서가 번호를 유지한다. 숫자 뒤 괄호 표기처럼 권수와 제목을 확정할 수 없는 잠실 항목은 업로드 전에 운영자 검토 대상으로 중단한다.
