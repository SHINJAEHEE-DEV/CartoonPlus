...
> **기준 스펙**: 카툰플러스 서울대입구역점/잠실점/홍대점 운영 전환 완료 Handoff Spec  
> **최종 갱신일**: 2026-09-18
...
- **Cloudflare Pages + Supabase**: 개발·운영에서 월 고정 운영비 0원($0, 도메인 실비 제외) 인프라를 유지하며 SSG(Static Site Generation) 정적 사전 빌드로 SEO 및 성능 최적화.
...
- **원클릭 & 예약 매장 안내 방송**: Web Speech API 및 고음질 MP3 음원 하이브리드 기반 매장 전용 프리셋, 커스텀 방송, 매일/요일별/1회성 예약 방송 및 실행 기록(Broadcast Run) 관리.
- **검증 기반 게임 & 자동 만료 이벤트 관리**: 실물 확인된 게임만 노출(미확인 시 "점검 중" 표시), 이벤트 종료일 익일 자동 숨김.
- **안전한 직원 계정 승인 체계**: 이메일 수집 없이 이름, 희망 지점, 로그인 ID, 비밀번호, 전화번호 뒷 4자리로 가입 신청 → 관리자 승인제.
...
1. **고객 (Guest / 비회원)**
   - 별도 회원가입/로그인 없이 모든 공개 서비스(지점별 도서 검색 홈, 매장 소개, 게임 카탈로그, 이벤트, 메뉴/요금제, 도서 입고 신청) 이용.
2. **매장 직원 (Staff)**
   - 관리자 승인 완료 계정으로 로그인하여 소속 지점 도서 재고 관리, CSV 일괄 가져오기, 입고 신청 처리, 게임 검증, 이벤트 관리, 방송 송출 및 예약 관리 수행.
3. **점주 / 시스템 관리자 (Admin)**
   - Staff의 모든 권한을 포함하며, 전 지점 전환 관리, 직원 가입 승인/반려, 계정 비활성화, 임시 비밀번호 발급 등 계정 관리 총괄.
...
flowchart TD
    subgraph 고객_사이클 [고객 사이클]
        A1[지점별 도서 검색 홈 방문] --> A2[도서 검색 & 결과 확인]
    ...
    subgraph 직원_사이클 [직원 사이클]
        ...
        B1 --> B5[원클릭 TTS/MP3 방송 및 예약 방송 스케줄링]
        B1 --> B6[게임 실물 검증 & 매장 이벤트 등록/관리]
    end

    subgraph 관리자_사이클 [관리자 사이클]
        C1[최초 관리자 생성 Supabase] --> C2[직원 가입 신청 승인/지점배정/비활성화]
...
### 1단계 (MVP) & 2단계 (다지점 공개 운영) - _구현 완료_

- **공개 운영 지점**: 서울대입구역점(`snu`), 잠실점(`jamsil`), 홍대점(`hongdae`) 3개 지점 전면 가동.
  - `/stores/:slug` 및 기본 경로 호환 라우팅 제공.
  - 정적 사전 빌드(SSG)를 통해 20개 지점별/공개 페이지를 정적 HTML로 생성하여 검색 엔진 최적화(SEO) 및 로딩 성능 극대화.
- **고객 포털**:
  - 지점별 도서 검색 중심 홈 (공백 무시, 초성 검색, 텍스트 전용 카드, 30일 신간 배지)
  - 도서 입고 신청 모달 및 독립 라우트 (`/request`)
  - 지점별 엔터테인먼트 카탈로그 (검증된 게임만 노출, 미검증 시 점검 중 표시)
  - 지점별 매장 이벤트 (진행 중 이벤트 노출, 종료일 익일 자동 숨김, Supabase 동기화)
  - 통합 F&B 메뉴판 및 선/후불 이용 요금제 안내 (`store_content` JSONB 기반)
  - 지점별 매장 안내 (영업시간, 주소, 전화번호, 주차 가이드)
- **직원/관리자 콘솔**:
  - 지점 컨텍스트 기반 운영 대시보드
  - 소속 지점 지정 직원 계정 신청/승인/비활성화
  - 도서 재고 단건 등록/수정/영구 삭제 및 지점별 CSV 일괄 가져오기
  - 입고 신청 접수 → 주문완료 → 입고완료 → 재고 등록 연계 (지점별 분리)
  - 매장 안내 방송 (MP3 프리셋 및 Web Speech TTS, 예약 스케줄러, 방송 실행 기록 및 지점 격리)
  - 게임 실물 검증 및 이벤트 등록/수정/삭제 (Supabase `store_events` 연동)
- **인프라**: Cloudflare Pages + Supabase ($0 고정비, 도메인 실비 제외)

### 3단계 (매장 운영 자동화) - _향후 로드맵_
...
### 4.3 UI 재설계 제외 범위 (Out of Scope)

- XLSX 파일 직접 파싱/가져오기 (Caspio 호환 CSV 사용)
- 별도 공지 작성·게시·만료 관리 (고정 안내는 이벤트 화면에서 통합 제공)
- 확인되지 않은 가격, 게임 수, 시설, 룸 예약, 인기 지표의 임의 고객 노출
...
- **예약 방송(Scheduled Broadcast)**:
  ...
  - 현재 방송 중지·대기 방송 취소를 제공한다. 실패 방송은 실행 기록만 남기고 재시도하지 않는다.
  - PC 종료 등으로 놓친 예약은 미실행으로 표시하고 나중에 자동 송출하지 않는다.
  - 프리셋 삭제 시 연결 예약을 보여주고 함께 삭제한다.
- **방송 실행 기록(Broadcast Run)**:
  - 상태: 대기·성공·실패·미실행을 기록한다. 예약·프리셋을 삭제해도 당시 문구·실행 시각·결과를 유지한다.
