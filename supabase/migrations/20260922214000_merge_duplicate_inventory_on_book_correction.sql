-- Keep the edited inventory record when its corrected title already exists in the same store.
create or replace function public.update_inventory_for_store(
  ...
  p_merge_duplicate boolean default false
)
returns uuid
language plpgsql
security definer
...
begin
  ...
  if v_existing_book_id is not null and v_existing_book_id <> v_current_book_id then
    select id
    into v_duplicate_inventory_id
    from public.book_inventories
    where store_id = v_store_id and book_id = v_existing_book_id
    for update;

    if v_duplicate_inventory_id is not null then
      if not p_merge_duplicate then
        raise exception 'duplicate inventory exists for this store';
      end if;
      delete from public.book_inventories where id = v_duplicate_inventory_id;
    end if;

    update public.book_inventories
    set book_id = v_existing_book_id
    where id = p_inventory_id;
  else
...
grant execute on function public.update_inventory_for_store(uuid, text, text, text, integer, text, boolean) to authenticated;
