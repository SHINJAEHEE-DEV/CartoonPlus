...
export function BookSearchPage({ books, isLoading = false }: BookSearchPageProps) {
  ...
  return (
    <div
      className="book-search-page"
      style={{ display: 'flex', flexDirection: 'column', gap: '24px', minHeight: '80vh' }}
    >
      {/* 상단 검색 컨트롤 카드 */}
      <section className="search-box-card">
        <div>
          ...
          >
            공백을 무시하고, 초성만 입력해도 찾아 드립니다. 결과에서 보유 권수와 서가 위치를 바로
            확인하세요.
          </p>
        </div>
        ...
        <div className="genre-select-mobile">
          ...
          >
            <span style={{ display: 'inline-flex', alignItems: 'center', gap: '6px' }}>
              ...
              <span>
                장르: <strong>{selectedGenre === '전체' ? '전체 장르' : selectedGenre}</strong>
              </span>
            </span>
            <span
              style={{
      ...
      >
          ...
          {isLoading ? (
            ...
            <>
              ...
              {filteredBooks.length > PAGE_SIZE && (
                <span
                  style={{ fontSize: '12px', fontWeight: 600, color: '#8A8175', marginLeft: '6px' }}
                >
        ...
        <div style={{ fontSize: '12px', fontWeight: 600, color: '#8A8175' }}>실시간 재고 기준</div>
      </div>
      ...
      {isLoading ? (
        ...
        <section className="empty-search-box" aria-live="polite">
          <img src={MASCOT_ASSETS.thinking} alt="고민하는 마스코트" />
