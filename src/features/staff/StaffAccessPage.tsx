...
export function StaffAccessPage() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    else { await signInStaff(String(form.get('loginId')),String(form.get('password'))); window.location.assign('/staff/dashboard'); } } catch(error) { setMessage(error instanceof Error ? error.message : '처리에 실패했습니다.'); }
  }
